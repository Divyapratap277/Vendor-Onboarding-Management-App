...
const AdminLoginPage = () => {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    setErrorMessage('');
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URI}/api/users/login`, data);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col justify-center items-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-md mx-auto">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-extrabold text-gray-900 mb-3">Admin Portal</h1>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-7">
              <div className="space-y-2">
                ...
                <Controller
                  name="email"
                  control={control}
                  render={({ field }) => <Input id="email" type="email" placeholder="admin@example.com" {...field} className="py-3 px-4 text-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg" />}
                />
